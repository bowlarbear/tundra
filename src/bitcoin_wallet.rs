...
pub async fn init_bitcoind() ->Result<String, String> {
	...
	let bitcoin_tar = std::path::Path::new(&(get_home().unwrap()+"/passport-vault/bitcoin-25.0-x86_64-linux-gnu.tar.gz")).exists();
	if bitcoin_tar == false{
		let output = Command::new("wget").args(["-O", &(get_home().unwrap()+"/passport-vault/bitcoin-25.0-x86_64-linux-gnu.tar.gz"),"https://bitcoincore.org/bin/bitcoin-core-25.0/bitcoin-25.0-x86_64-linux-gnu.tar.gz"]).output().unwrap();
		if !output.status.success() {
	...
	if bitcoin == false{
		let output = Command::new("tar").args(["-xzf", &(get_home().unwrap()+"/passport-vault/bitcoin-25.0-x86_64-linux-gnu.tar.gz"), "-C", &get_home().unwrap()]).output().unwrap();
		if !output.status.success() {
