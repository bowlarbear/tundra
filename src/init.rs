...
//     let arc = std::path::Path::new(&(get_home().unwrap()+"/tundra-tmp")).exists();
// 	if arc == false {
// 		let output = Command::new("mkdir").arg(&(get_home().unwrap()+"/tundra-tmp")).output().unwrap();
// 		if !output.status.success() {
// 			return Err(format!("ERROR in pre_install with creating tundra-tmp directory = {}", std::str::from_utf8(&output.stderr).unwrap()));
// 		}
// 	}
...
// 	//https://github.com/bowlarbear/tundra-iso-builder 
// 	//users should create the tundra-tmp directory in their home dir and move the tundra-ubuntu-22.04-amd64.iso created with the utility into it

//     //check if ubuntu iso already exists, and if not, obtain
// 	// let b = std::path::Path::new(&(get_home().unwrap()+"/tundra-tmp/tundra-ubuntu-22.04-amd64.iso")).exists();
//     // if b == false{
//     // 	let output = Command::new("wget").args(["-O", "tundra-ubuntu-22.04-amd64.iso", "https://old-releases.ubuntu.com/releases/jammy/ubuntu-22.04-desktop-amd64.iso"]).output().unwrap();
//     // 	if !output.status.success() {
...
// 		.args(["dd", &("if=".to_string()+&get_home().unwrap()+"/tundra-tmp/tundra-ubuntu-22.04-amd64.iso"), &("of=".to_string()+&target), "bs=16M", "oflag=sync", "status=progress"])
...
//     	}
// 	}
// 	//TODO remove the tundra temp dir. keep it for now until bulk of dev work and testing is finished
...
// //build the tundra iso for initial flash
// #[tauri::command]
...
// 	let arc = std::path::Path::new(&(get_home().unwrap()+"/tundra")).exists();
// 	if arc == false {
// 		let output = Command::new("mkdir").arg(&(get_home().unwrap()+"/tundra")).output().unwrap();
// 		if !output.status.success() {
// 			return Err(format!("ERROR in init iso with creating tundra directory = {}", std::str::from_utf8(&output.stderr).unwrap()));
// 		}
// 	}
...
// 	env::set_current_dir(&(get_home().unwrap()+"/tundra"));
...
// 	let b = std::path::Path::new(&(get_home().unwrap()+"/tundra/ubuntu-22.04.3-desktop-amd64.iso")).exists();
// 	let c = std::path::Path::new(&(get_home().unwrap()+"/tundra/bitcoin-25.0-x86_64-linux-gnu.tar.gz")).exists();
// 	if b == false{
...
// 	let d = std::path::Path::new(&(get_home().unwrap()+"/tundra/scripts")).exists();
// 	if d == false {
// 		let output = Command::new("mkdir").arg(&(get_home().unwrap()+"/tundra/scripts")).output().unwrap();
// 		if !output.status.success() {
// 			return Err(format!("ERROR in creating the scripts directory {}", std::str::from_utf8(&output.stderr).unwrap()));
// 		} 
// 	}
// 	//create sed1 script
// 	let file = File::create(&(get_home().unwrap()+"/tundra/scripts/sed1.sh")).unwrap();
...
// 	}
// 	//create sed2 script
// 	let file = File::create(&(get_home().unwrap()+"/tundra/scripts/sed2.sh")).unwrap();
...
// 	let output = Command::new("bash").args([&(get_home().unwrap()+"/tundra/scripts/sed1.sh")]).output().unwrap();
// 	if !output.status.success() {
// 		return Err(format!("ERROR in running sed1 {}", std::str::from_utf8(&output.stderr).unwrap()));
// 	} 
// 	//verify that the iso has been modified
// 	let exists = Path::new(&(get_home().unwrap()+"/tundra/persistent-ubuntu1.iso")).exists();
// 	if !exists {
...
// 	let output = Command::new("bash").args([&(get_home().unwrap()+"/tundra/scripts/sed2.sh")]).output().unwrap();
// 	if !output.status.success() {
// 		return Err(format!("ERROR in running sed2 {}", std::str::from_utf8(&output.stderr).unwrap()));
// 	} 
// 	//verify that the iso has been modified
// 	let exists = Path::new(&(get_home().unwrap()+"/tundra/persistent-ubuntu.iso")).exists();
// 	if !exists {
...
// 	//TODO check for existing tundra installation and return error if found
...
// 	//burn iso with dd
// 	let output = Command::new("sudo")
//         .args(["dd", &("if=".to_string()+&get_home().unwrap()+"/tundra/persistent-ubuntu.iso"), &("of=".to_string()+&target), "bs=4M", "oflag=sync", "status=progress"])
//         .output()
//         .unwrap();
// 		if !output.status.success() {
...
// 	let script = std::path::Path::new(&(get_home().unwrap()+"/tundra/scripts/create_partition.sh")).exists();
// 	if script == true{
// 		//remove stale create_partition.sh script
// 		let output = Command::new("sudo").args(["rm", &(get_home().unwrap()+"/tundra/scripts/create_partition.sh")]).output().unwrap();
// 		if !output.status.success() {
...
// 	let file = File::create(&(get_home().unwrap()+"/tundra/scripts/create_partition.sh")).unwrap();
...
// 	let output = Command::new("sudo").args(["chmod", "+x", &(get_home().unwrap()+"/tundra/scripts/create_partition.sh")]).output().unwrap();
// 	if !output.status.success() {
...
// 	Command::new("bash").arg(&(get_home().unwrap()+"/tundra/scripts/create_partition.sh")).output().unwrap();
// 	if !output.status.success() {
...
// 	let output = Command::new("rm").arg(&(get_home().unwrap()+"/tundra/scripts/create_partition.sh")).output().unwrap();
// 	if !output.status.success() {
...
// 	}
// 	//make the user directory and tundra directory
// 	let output = Command::new("mkdir").args(["-p", &("/media/".to_string()+&get_user().unwrap()+"/writable/upper/home/ubuntu/tundra")]).output().unwrap();
// 		if !output.status.success() {
...
// 	let output = Command::new("cp").args([&(get_home().unwrap()+"/tundra/genisoimage_9%3a1.1.11-3.2ubuntu1_amd64.deb"), &("/media/".to_string()+&get_user().unwrap()+"/writable/upper/home/ubuntu/dependencies")]).output().unwrap();
// 	if !output.status.success() {
...
// 	let output = Command::new("cp").args([&(get_home().unwrap()+"/tundra/ssss_0.5-5_amd64.deb"), &("/media/".to_string()+&get_user().unwrap()+"/writable/upper/home/ubuntu/dependencies")]).output().unwrap();
// 	if !output.status.success() {
...
// 	let output = Command::new("cp").args([&(get_home().unwrap()+"/tundra/wodim_9%3a1.1.11-3.2ubuntu1_amd64.deb"), &("/media/".to_string()+&get_user().unwrap()+"/writable/upper/home/ubuntu/dependencies")]).output().unwrap();
// 	if !output.status.success() {
...
// 	let output = Command::new("cp").args([&(get_home().unwrap()+"/tundra/libqrencode4_4.1.1-1_amd64.deb"), &("/media/".to_string()+&get_user().unwrap()+"/writable/upper/home/ubuntu/dependencies")]).output().unwrap();
// 	if !output.status.success() {
...
// 	let output = Command::new("cp").args([&(get_home().unwrap()+"/tundra/qrencode_4.1.1-1_amd64.deb"), &("/media/".to_string()+&get_user().unwrap()+"/writable/upper/home/ubuntu/dependencies")]).output().unwrap();
// 	if !output.status.success() {
// 		return Err(format!("ERROR in create_bootable with copying qrencode = {}", std::str::from_utf8(&output.stderr).unwrap()));
// 	}
// 	//copying dependencies xclip
// 	let output = Command::new("cp").args([&(get_home().unwrap()+"/tundra/xclip_0.13-2_amd64.deb"), &("/media/".to_string()+&get_user().unwrap()+"/writable/upper/home/ubuntu/dependencies")]).output().unwrap();
// 	if !output.status.success() {
...
// 	let output = Command::new("cp").args([&(get_home().unwrap()+"/tundra/tor_0.4.6.10-1_amd64.deb"), &("/media/".to_string()+&get_user().unwrap()+"/writable/upper/home/ubuntu/dependencies")]).output().unwrap();
// 	if !output.status.success() {
...
// 	let output = Command::new("cp").args([&(get_home().unwrap()+"/tundra/libzbar0_0.23.92-4build2_amd64.deb"), &("/media/".to_string()+&get_user().unwrap()+"/writable/upper/home/ubuntu/dependencies")]).output().unwrap();
// 	if !output.status.success() {
...
// 	let output = Command::new("cp").args([&(get_home().unwrap()+"/tundra/imagemagick-6-common_8%3a6.9.11.60+dfsg-1.3ubuntu0.22.04.3_all.deb"), &("/media/".to_string()+&get_user().unwrap()+"/writable/upper/home/ubuntu/dependencies")]).output().unwrap();
// 	if !output.status.success() {
...
// 	let output = Command::new("cp").args([&(get_home().unwrap()+"/tundra/libaom3_3.3.0-1_amd64.deb"), &("/media/".to_string()+&get_user().unwrap()+"/writable/upper/home/ubuntu/dependencies")]).output().unwrap();
// 	if !output.status.success() {
...
// 	let output = Command::new("cp").args([&(get_home().unwrap()+"/tundra/libdav1d5_0.9.2-1_amd64.deb"), &("/media/".to_string()+&get_user().unwrap()+"/writable/upper/home/ubuntu/dependencies")]).output().unwrap();
// 	if !output.status.success() {
...
// 	let output = Command::new("cp").args([&(get_home().unwrap()+"/tundra/libde265-0_1.0.8-1_amd64.deb"), &("/media/".to_string()+&get_user().unwrap()+"/writable/upper/home/ubuntu/dependencies")]).output().unwrap();
// 	if !output.status.success() {
...
// 	let output = Command::new("cp").args([&(get_home().unwrap()+"/tundra/libx265-199_3.5-2_amd64.deb"), &("/media/".to_string()+&get_user().unwrap()+"/writable/upper/home/ubuntu/dependencies")]).output().unwrap();
// 	if !output.status.success() {
...
// 	let output = Command::new("cp").args([&(get_home().unwrap()+"/tundra/libheif1_1.12.0-2build1_amd64.deb"), &("/media/".to_string()+&get_user().unwrap()+"/writable/upper/home/ubuntu/dependencies")]).output().unwrap();
// 	if !output.status.success() {
...
// 	let output = Command::new("cp").args([&(get_home().unwrap()+"/tundra/liblqr-1-0_0.4.2-2.1_amd64.deb"), &("/media/".to_string()+&get_user().unwrap()+"/writable/upper/home/ubuntu/dependencies")]).output().unwrap();
// 	if !output.status.success() {
...
// 	let output = Command::new("cp").args([&(get_home().unwrap()+"/tundra/libmagickcore-6.q16-6_8%3a6.9.11.60+dfsg-1.3ubuntu0.22.04.3_amd64.deb"), &("/media/".to_string()+&get_user().unwrap()+"/writable/upper/home/ubuntu/dependencies")]).output().unwrap();
// 	if !output.status.success() {
...
// 	let output = Command::new("cp").args([&(get_home().unwrap()+"/tundra/libmagickwand-6.q16-6_8%3a6.9.11.60+dfsg-1.3ubuntu0.22.04.3_amd64.deb"), &("/media/".to_string()+&get_user().unwrap()+"/writable/upper/home/ubuntu/dependencies")]).output().unwrap();
// 	if !output.status.success() {
...
// 	let output = Command::new("cp").args([&(get_home().unwrap()+"/tundra/zbar-tools_0.23.92-4build2_amd64.deb"), &("/media/".to_string()+&get_user().unwrap()+"/writable/upper/home/ubuntu/dependencies")]).output().unwrap();
// 	if !output.status.success() {
// 		return Err(format!("ERROR in create_bootable with copying zbar-tools = {}", std::str::from_utf8(&output.stderr).unwrap()));
// 	}
// 	//copy over tundra binary
// 	let output = Command::new("cp").args([format!("{}/Tundra", initial_cwd.unwrap()), format!("/media/{}/writable/upper/home/ubuntu", get_user().unwrap())]).output().unwrap();
// 	if !output.status.success() {
// 		return Err(format!("ERROR in create_bootable with copying tundra binary = {}", std::str::from_utf8(&output.stderr).unwrap()));
// 	}
// 	//extract bitcoin core
// 	let output = Command::new("tar").args(["-xzf", &(get_home().unwrap()+"/tundra/bitcoin-25.0-x86_64-linux-gnu.tar.gz"), "-C", &("/media/".to_string()+&get_user().unwrap()+"/writable/upper/home/ubuntu")]).output().unwrap();
// 	if !output.status.success() {
