...
// 	//check if the ramdisk already exists and has been used by Tundra this session
...
// 			}
// 		//open tundra directory permissions (putting this here because this script always runs at startup)
// 		let output = Command::new("sudo").args(["chmod", "777", &(get_home().unwrap()+"/tundra")]).output().unwrap();
// 		if !output.status.success() {
// 			return Err(format!("ERROR in opening file permissions of $HOME/tundra = {}", std::str::from_utf8(&output.stderr).unwrap()));
// 		}
...
// 	let combine_script = std::path::Path::new(&(get_home().unwrap()+"/tundra/combine-shards.sh")).exists();
// 	if combine_script == true{
// 		let output = Command::new("sudo").args(["rm", &(get_home().unwrap()+"/tundra/combine-shards.sh")]).output().unwrap();
// 		if !output.status.success() {
...
// 	let file = File::create(&(get_home().unwrap()+"/tundra/combine-shards.sh")).unwrap();
...
// 		.args([get_home().unwrap()+"/tundra/combine-shards.sh"])
// 		.output()
...
// #[tauri::command]
// async fn enable_webcam_qr_scan() -> Result<String, String> {
// 	let script = std::path::Path::new(&(get_home().unwrap()+"/tundra/enable-webcam-scan.sh")).exists();
// 	if script == false{
// 		//create enable-webcam-scan.sh script
// 		let file = File::create(&(get_home().unwrap()+"/tundra/enable-webcam-scan.sh")).unwrap();
...
// 	let output = Command::new("bash").args([&(get_home().unwrap()+"/tundra/enable-webcam-scan.sh")]).output().unwrap();
// 	if !output.status.success() {
