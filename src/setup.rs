...
//create passport-vault descriptors
...
pub async fn create_setup_cd() -> Result<String, String> {
	...
	let file = File::create(&(get_home().unwrap()+"/passport-vault/create-setup-cd.sh")).unwrap();
	...
	let output = Command::new("bash").args([&(get_home().unwrap()+"/passport-vault/create-setup-cd.sh")]).output().unwrap();
	if !output.status.success() {
